...
//             $image = Mage::getBaseDir('media') . '/sales/store/logo/' . $image;
    	    $image = Mage::getBaseDir('media') . '/logo_email.png';
            if (is_file($image)) {
...
//         }
    ...
    protected function insertRBLogo(&$page, $store = null)
    {
    	//         $image = Mage::getStoreConfig('sales/identity/logo', $store);
    	//         if ($image) {
    	//             $image = Mage::getBaseDir('media') . '/sales/store/logo/' . $image;
    		$image = Mage::getBaseDir('media') . '/rb.jpg';
    		if (is_file($image)) {
    			$image = Zend_Pdf_Image::imageWithPath($image);
    			$page->drawImage($image, 500, 800, 560, 830);
    		}
    		//         }
    		return $page;
    	}
...
//         foreach (explode("\n", Mage::getStoreConfig('sales/identity/address', $store)) as $value){
        foreach (explode("\n", "Reckitt Benckiser (Hong Kong) Ltd Unit 1503-7\n 15th floor Millennium City 6 392 Kwun Tong\n Road Kwun Tong, Kowloon, Hong Kong") as $value){
            if ($value!=='') {
                $page->drawText(trim(strip_tags($value)), 250, $this->y, 'UTF-8');
                $this->y -=8;
            }
        }
        //return $page;
    ...
    protected function insertOrder(&$page, $obj, $putOrderId = true)
    {
        ...
        if ($putOrderId) {
            $page->drawText(Mage::helper('sales')->__('Order # ').$order->getRealOrderId(), 35, 768, 'UTF-8');
        }
        //$page->drawText(Mage::helper('sales')->__('Order Date: ') . date( 'D M j Y', strtotime( $order->getCreatedAt() ) ), 35, 760, 'UTF-8');
        $page->drawText(Mage::helper('sales')->__('Order Date: ') . Mage::helper('core')->formatDate($order->getCreatedAtStoreDate(), 'medium', false), 35, 756, 'UTF-8');
        ...
        $page->drawText(Mage::helper('sales')->__('SOLD TO:'), 35, 720 , 'UTF-8');

        if (!$order->getIsVirtual()) {
            $page->drawText(Mage::helper('sales')->__('SHIP TO:'), 285, 720 , 'UTF-8');
        }
        else {
            $page->drawText(Mage::helper('sales')->__('Payment Method:'), 285, 720 , 'UTF-8');
        }

        if (!$order->getIsVirtual()) {
            $y = 710 - (max(count($billingAddress), count($shippingAddress)) * 10 + 5);
        }
        else {
            $y = 710 - (count($billingAddress) * 10 + 5);
        }
        ...
        if (!$order->getIsVirtual()) {
            ...
            $page->drawText("Estimated Delivery Time: 3 Days", 285, $yShipments-14, 'UTF-8');
            $yShipments -=10;
    ...
    public function drawLineBlocks(Zend_Pdf_Page $page, array $draw, array $pageSettings = array())
    {
        foreach ($draw as $itemsProp) {
            ...
            foreach ($lines as $line) {
                $maxHeight = 0;
                foreach ($line as $column) {
                    $fontSize = empty($column['font_size']) ? 15 : $column['font_size'];
