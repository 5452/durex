...
//        //this.accordion.openNextSection(true);
    ...
    //alert(response.update_section.html);
        ...
        if (response.goto_section) {
            if(response.goto_section=="shipping"){shipping.save(1);}
            else{
            this.reloadProgressBlock();
            temp = response.goto_section;
            if(response.goto_section=="shipping_method"){temp = "billing";}
            if(response.goto_section=="payment"){temp = "review";}
            this.gotoSection(temp);
            return true;
            }
        }
...
//        $('billing:use_for_shipping_yes').checked = flag;
    ...
    save: function(type){
        ...
        }else{checkout.setLoadWaiting(false);}
        }
        else{
        if (checkout.loadWaiting!=false) return;
        var validator = new Validation(this.form);
